...
def index():
    ...
    try:
        projects = db_session.query(Project).filter_by(uid=user.uid).all()
        items = db_session.query(Item).filter_by(uid=user.uid, state="Open").all()
...
def project(pid):
    ...
    try:
        project = db_session.query(Project).filter_by(pid=pid, uid=user.uid).one()
        projects = db_session.query(Project).filter_by(uid=user.uid).all()
        links = db_session.query(Link).filter_by(pid=pid).all()
        open_items = db_session.query(Item).filter_by(pid=pid, state="Open").all()
        resolved_items = db_session.query(Item).filter_by(pid=pid, state="Resolved").all()
...
def account():
    ...
    try:
        projects = db_session.query(Project).filter_by(uid=user.uid).all()
        open_items = db_session.query(Item).filter_by(state="Open", uid=user.uid).all()
        resolved_items = db_session.query(Item).filter_by(state="Resolved", uid=user.uid).all()
...
def delete_project(pid):
    db_session = Session()
    project = db_session.query(Project).filter_by(pid=pid).first()
    items = db_session.query(Item).filter_by(pid=pid).all()
    print items
...
def add_item():
    db_session = Session()
    user = db_session.query(User).filter_by(email = session['email']).first().uid
...
def delete_link(id):
    db_session = Session()
    link = db_session.query(Link).filter_by(id=id).one()
